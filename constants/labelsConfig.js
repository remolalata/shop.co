export const HOME_TITLE = "SHOP.CO";
export const PRODUCT_TITLE = "SHOP.CO - Product";
export const SHOP_CO = "Shop Co";
export const CURRENCY_CODE = "$";
export const HEADING_LABEL = "FIND CLOTHES THAT MATCHES YOUR STYLE";
export const SUBHEADING_LABEL = "Browse through our diverse range of meticulously crafted garments, designed to bring out your individuality and cater to your sense of style.";
export const SHOP_NOW = "Show Now";
export const ALL_PRODUCTS = "ALL PRODUCTS";
export const NEW_ARRIVALS = "NEW ARRIVALS";
export const TOP_SELLING = "TOP SELLING";
export const CASUAL = "Casual";
export const VIEW_ALL = "View All";
export const BROWSE_BY_DRESS_STYLE = "BROWSE BY DRESS STYLE";
export const OUR_HAPPY_CUSTOMERS = "OUR HAPPY CUSTOMERS";
export const FOOTER_INFORMATION = "We have clothes that suits your style and which you're proud to wear. From women to men.";
export const FOOTER_COPYRIGHT = "Shop.co © 2000-2023, All Rights Reserved";
export const SOCIAL_MEDIAS_ICON_ALT = "Social Media";
export const NEWSLETTER_HEADING = "STAY UPTO DATE ABOUT OUR LATEST OFFERS";
export const NEWSLETTER_INPUT_PLACEHOLDER = "Enter your email address";
export const NEWSLETTER_BUTTON = "Subscribe to Newsletter";
export const FILTERS_LABEL = "Filters";
export const FILTERS_PRICE = "Price";
export const FILTERS_COLORS = "Colors";
export const FILTERS_SIZES = "Size";
export const FILTERS_STYLES = "Dress Style";
export const APPLY_FILTER_LABEL = "Apply Filter";
export const PRODUCT_RESULT_SUMMARY_TEXT = "Showing 1-10 of 100 Products";